...
<div class="container my-5">
    ...
    <a href="categoryadd.php">
        ...
        <button type="button" class="btn btn-primary my-5 col-md-12">Yeni Kategori Ekle</button>
...
$rows = $db -> query("SELECT * FROM category",PDO::FETCH_ASSOC);
$count=0;
foreach ($rows as $row){
    //print_r($row);
    $count++;
    ?>
            <tr>
                ...
                <td>
                    <a href="categoryupdate.php?category_id=<?= $row['category_id'] ?>">
                        <button type="button" class="btn btn-outline-primary">Güncelle</button>
                    </a>
                    <a href="delete.php?action=categoryDelete&category_id=<?= $row['category_id'] ?>">
                        <button name="category_delete" type="button" class="btn btn-outline-danger">Sil</button>
                    </a>
